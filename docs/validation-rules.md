...
| `JM-RINGFAM-001` | `ringFamily.variant` | The declared variant belongs to a different family than `jewelry.style` names. Refused rather than resolved by precedence: two authorities over one design have no determinate resolution. | error |
| `JM-RINGFAM-002` | the pre-empted block | The ring family would have derived a block the document declares itself. The declared values are used unchanged and the family derived none; reported so an author who wrote both learns which won. | information |
| `JM-RINGFAM-003` | `ringFamily.params.*` | The chosen variant does not read a parameter the author set. The value stays in the document and has no effect on the geometry — reported rather than silently ignored. | information |
| `JM-RINGFAM-004` | `ringFamily.params.splitSeparationMm` / `bypassSeparationMm` / `signetTableWidthMm` | The requested structure is not geometrically possible: a rail separation at or above the band's width leaves no rail, because the rails SHARE that width. ARITHMETIC, not a judgment about how thin a rail may be — a 0.15 mm rail is thin and is not refused. A signet table narrower than the band is a warning rather than an error: valid, and probably not intended. | error / warning |
| `JM-RINGFAM-005` | `jewelry.style` | The resolved variant's capability status is PARTIAL rather than CURRENT, with what is missing. Reported so a caller never has to read the capability registry to learn that a family it just used is incomplete. | information |
