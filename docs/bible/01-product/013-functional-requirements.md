...
| JM-FR-046 | The system shall generate real, deterministic prong geometry in each of four styles, while leaving every `ROUND_PRONG` request byte-identical to the pre-Sprint-23 cylinder. | current | `backend/jewelmind/setting/prong_styles.py`, `test_setting_v2.py` (96 tests) |
| JM-FR-047 | The system shall generate real, deterministic head geometry in each of four architectures, each one connected solid spanning the same vertical extent, while leaving every `BASKET` request byte-identical to the pre-Sprint-23 wall. | current | `backend/jewelmind/setting/head.py`, `test_setting_v2.py`, `goldens/` (39 cases, zero updates) |
| JM-FR-048 | Head construction shall be category-neutral: no head builder may read a band, a ring size or any category field. | current | `backend/jewelmind/setting/head.py`, `test_setting_system_no_ring_dependency.py` |
| JM-FR-049 | A setting shall accept explicit prong positions and per-group style overrides, and shall refuse a layout that mixes explicit and derived positions or names a non-existent position. | current | `backend/jewelmind/setting/prong.py`, `test_setting_v2.py::TestExplicitPositions` |
| JM-FR-050 | A setting shall report, deterministically and by identifier, which stone instances each generated component serves, without importing the arrangement layer. | current | `backend/jewelmind/setting/models.py`, `test_setting_v2.py::TestStoneInstanceMapping` |
| JM-FR-051 | Opt-in seat relief shall remove metal where the stone sits by CUTTING the stone solid out of production metal, never by fusing it, and shall be off by default. | current | `backend/jewelmind/setting/seat.py`, `test_setting_v2.py::TestSeatRelief` |
| JM-FR-052 | The system shall validate advanced head and prong configurations structurally — required parameters, field applicability, operation feasibility — without any professional or manufacturing threshold. | current | `backend/jewelmind/validation/engine.py::_setting_v2_rules`, `test_setting_v2.py::TestSettingV2Rules` |
| JM-FR-053 | A design that leaves the advanced setting fields on their defaults shall generate exactly as it did before they existed. | current | `test_setting_v2.py::TestBackwardCompatibility`, the 39-case Golden suite |
| JM-FR-054 | A design shall be able to declare a multi-stone family — three-stone, toi-et-moi, cluster or centre-with-accents — describing its participating stones, their semantic roles and their relationships, without restating what any stone is or is made of. | current | `backend/jewelmind/family/models.py`, `test_multi_stone_families.py` (116 tests) |
| JM-FR-055 | A family shall compile into arrangement primitives, so the Stone Arrangement Engine remains the sole authority on stone placement, and a document declaring both a family and an explicit arrangement shall be refused. | current | `backend/jewelmind/family/compile.py`, `family/effective.py`, `test_multi_stone_families.py::TestArrangementIsTheAuthority` |
| JM-FR-056 | The family layer shall be category- and kernel-neutral: no family module may import a jewelry category, a geometry module, the Setting System or the CAD kernel. | current | `backend/jewelmind/family/`, `test_multi_stone_families.py::test_the_family_package_never_constructs_geometry` |
| JM-FR-057 | Family compilation shall be deterministic: the same family shall compile to byte-identical output on every run, and member order shall not affect the result. | current | `test_multi_stone_families.py::TestDeterminism` |
| JM-FR-058 | The system shall build a real stone solid for every resolved stone instance, applying its own placement, scale and orientation, while leaving a single-stone design's geometry untouched. | current | `backend/jewelmind/geometry/stone/instance.py`, `stone/placement.py`, `test_multi_stone_families.py::TestGeometry`, `goldens/` (39 cases, zero updates) |
| JM-FR-059 | Every generated stone component shall remain individually identifiable, classified as a stone reference rather than production metal, and required by Geometry Inspection. | current | `backend/jewelmind/geometry/inspection/assembly.py`, `test_multi_stone_families.py::TestInspection` |
| JM-FR-060 | Family definitions shall be validated structurally — one placement authority, accepted roles, correct cardinality, resolvable references — without any proportion, spacing or settability threshold. | current | `backend/jewelmind/validation/engine.py::_family_rules`, `test_multi_stone_families.py::TestFamilyRules` |
| JM-FR-061 | The system shall report, on every multi-member family, that a setting is generated only for the primary stone, rather than implying that every stone is held. | current | `JM-FAMILY-005`, `backend/jewelmind/arrangement/compile.py`, `specs/family/v1/test-vectors/compilation-vectors.json` |
