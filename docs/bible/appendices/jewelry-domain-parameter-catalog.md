...
| `jewelry.style` | string | — | `solitaire` \| `three_stone` \| `halo` \| `split_shank` \| `bypass` \| `signet` | `"solitaire"` | direct | THE RING FAMILY. Selects which generator runs and which variant set is available; with `ringFamily.variant` it determines every derived block. Was `fixed "solitaire"` from Sprint 16 until Sprint 28. | `JM-RINGFAM-001`, `JM-RINGFAM-005` | Select (`ring-family`) | not_required |
...
| `band.architecture` | string | — | `"UNIFORM"` \| `"SPLIT"` \| `"BYPASS"` | `"UNIFORM"` | direct | selects the shank construction: the pre-Sprint-17 revolve, two joined rails, or one open rail past a full turn (Sprint 28) | `JM-RINGFAM-004` | derived from the ring-family variant; no direct control | preliminary |
| `band.splitSeparation` | float | mm | `> 0` (schema-level) | `1.2` | direct | the axial gap between a split shank's rails AND, because they SHARE the band's width, the rails' own width `(width − separation) / 2` (Sprint 28) | `JM-RINGFAM-004` | Numeric input (`ring-family-split-separation`) | preliminary |
| `band.splitJoinSpan` | float | degrees | `15`–`350` (schema-level) | `200` | direct | how much of the ring's bottom the two rails are joined over (Sprint 28) | none directly | Numeric input (`ring-family-split-join`) | preliminary |
| `band.bypassSeparation` | float | mm | `> 0` (schema-level) | `1.0` | direct | the axial distance a bypass rail travels across its sweep, and therefore the clearance between its two passes (Sprint 28) | `JM-RINGFAM-004` | Numeric input (`ring-family-bypass-separation`) | preliminary |
| `band.bypassOverlap` | float | degrees | `6`–`180` (schema-level) | `60` | direct | how far past a full turn the bypass rail travels — what makes its ends pass rather than meet (Sprint 28) | none directly | Numeric input (`ring-family-bypass-overlap`) | preliminary |
| `ringFamily.variant` | string \| absent | — | 13 members (`ring_family/models.py`) | absent (`null`) | INDIRECT — derives other fields | which variant resolves, and therefore the shank/shoulder/body architectures, the head-height factor and the `family`/`halo`/`pave` blocks. `null` resolves to the family's default, which for `solitaire` reproduces the pre-Sprint-28 design exactly (Sprint 28) | `JM-RINGFAM-001`, `JM-RINGFAM-002`, `JM-RINGFAM-005` | Select (`ring-family-variant`) | not_required |
| `ringFamily.enabled` | bool | — | — | `true` | INDIRECT | `false` keeps the parameters and falls back to the family's default variant — a different state from declaring no block (Sprint 28) | `JM-RINGFAM-003` | derived; no direct control | not_required |
| `ringFamily.params.*` | float / int / bool / string | mm, degrees or dimensionless factors | per field (`ring_family/models.py`) | per field | INDIRECT — MODULATES document values | every derived value. Each factor MULTIPLIES what the document already states rather than replacing it, which is what keeps `ring.innerDiameter` and `stone.diameter` driving the geometry (Sprint 28) | `JM-RINGFAM-003`, `JM-RINGFAM-004` | Numeric/select inputs, shown only for the parameters the chosen variant reads | preliminary |
| `ringFamily.label` | string \| null | — | ≤ 120 chars | `null` | none | metadata only; affects no geometry (Sprint 28) | none | none | not_required |
