---
id: JM-BIBLE-A87
title: "Appendix: Professional Rule Review Matrix"
version: 1.0.0
status: accepted
owner: JewelMind
...
# Appendix: Professional Rule Review Matrix

A compact, table-only re-statement of [`443-current-preliminary-rule-review-plan.md`](../15-professional-validation/443-current-preliminary-rule-review-plan.md), which itself reads directly from `specs/forge/v1/current-rule-registry.json` (real `ruleId`/`classification`/`professionalValidationStatus`) and `backend/jewelmind/validation/engine.py` (real rule statements). 21 rules total: 16 need eventual professional review, 5 do not — see [`professional-validation-status-matrix.md`](professional-validation-status-matrix.md) (`JM-BIBLE-A86`) for the full 21-row status crosswalk.
...
| JM-PRONG-003 | 4 prongs are blocked when stone diameter exceeds 8mm. | PROTOTYPE_HEURISTIC | STONE_SETTER | High (Priority 1 target, see `444-current-solitaire-review-plan.md`) | STONE_SETTING_TEST, PROFESSIONAL_EXPERIENCE | Exactly 8.0mm and 8.1mm, both prong counts. |
...
Per [`444-current-solitaire-review-plan.md`](../15-professional-validation/444-current-solitaire-review-plan.md): setting/prong/basket rules (`JM-PRONG-*`, `JM-SETTING-*`) are Priority 1–2, band geometry (`JM-BAND-*`) is Priority 3, and the remaining preliminary Forge thresholds are Priority 5 — start where the current geometry is most simplified, not with every rule at once.

## Cross-references

- [`443-current-preliminary-rule-review-plan.md`](../15-professional-validation/443-current-preliminary-rule-review-plan.md) — the narrative source document this appendix compacts.
- [`professional-validation-status-matrix.md`](professional-validation-status-matrix.md) (`JM-BIBLE-A86`) — `professionalValidationStatus` ↔ `ValidationStatus` crosswalk for all 21 rules.
- [`419-rule-validation-process.md`](../15-professional-validation/419-rule-validation-process.md) — how one of these rules is actually reviewed once a reviewer is assigned.
