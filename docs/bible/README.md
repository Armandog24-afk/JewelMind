...
- **Sprint 15 — Geometry Quality & Golden Models v1**
  ([`17-geometry-quality/`](17-geometry-quality/)), a software
  regression layer built on top of Sprint 14's inspection facts: 9 real
  `GoldenModel` fixtures (`goldens/solitaire-v1/`) capture expected
  geometry for real solitaire variations, compared against on every
  regeneration via `backend/jewelmind/geometry_quality/`. A Golden
  Model is a software regression reference only — never a professional,
  manufacturing, or aesthetic claim; baseline changes require an
  explicit, human-invoked `accept --reason` — never an automatic
  overwrite by CI or an agent. Start there at
  [`17-geometry-quality/README.md`](17-geometry-quality/README.md).
- **Sprint 16 — Ring Architecture v2 / Multi-Category Ready**
  ([`18-ring-architecture/`](18-ring-architecture/)), the architectural
  boundary between JewelMind (the platform), a jewelry category (Ring),
  and a ring family (Solitaire) — Ring is no longer the platform's
  implicit root. A real, generic category dispatch boundary
  (`backend/jewelmind/jewelry_category/`) and a composable internal
  `RingDefinition` v2 (`backend/jewelmind/ring/`, built from the
  unmodified real JDL by an additive adapter) now sit between
  `ModelService.generate()` and the exact same, unmodified
  `build_solitaire_ring()` Sprint 5 built. No new jewelry category was
  implemented — a test-only dummy category proves the boundary is
  genuinely generic without ever reaching production. Zero Golden
  baseline updates were required. Start there at
  [`18-ring-architecture/README.md`](18-ring-architecture/README.md).
- **Sprint 17 — Band & Shank System v1**
  ([`19-shank/`](19-shank/)), the first real-geometry milestone since
  Atlas: a reusable, parametric Shank subsystem replaces the old
  uniform-only band, supporting real deterministic width and/or
  thickness taper anchored at the head. Every uniform-shank
  configuration uses the byte-identical pre-Sprint-17 construction —
  zero Golden regression for the 9 existing cases — while 3 new Golden
  cases prove real, non-uniform CAD geometry. Start there at
  [`19-shank/README.md`](19-shank/README.md).
- **Sprint 18 — Stone System v1**
  ([`20-stone/`](20-stone/)), the round-only `StoneReference` generalized
  into shared, **category-neutral** jewelry infrastructure: 7 shapes
  (round, oval, pear, emerald, cushion, princess, marquise) all
  generating real, deterministic CAD reference geometry, with round
  preserved byte-identically and zero Golden baseline updates. Stone
  System belongs to no jewelry category and provably never imports
  `jewelmind.ring`. Start there at
  [`20-stone/README.md`](20-stone/README.md).
- **Sprint 20 — Stone System v2**
  ([`22-stone-v2/`](22-stone-v2/)), the Stone System freed from a finite
  built-in shape list. 14 new native cuts (21 total), a second independent
  axis for the 3D reference profile (faceted / cabochon / spherical), and
  three escape hatches — custom outlines, measured stones, and imported
  CAD/mesh assets — so JewelMind can work with a stone that corresponds to no
  named cut. Start at [`22-stone-v2/README.md`](22-stone-v2/README.md).

- **Sprint 21 — Gem Identity & Material System v1**
  ([`23-gem-identity/`](23-gem-identity/)), a stone's semantic identity
  separated from its geometry. A gem registry (type level) plus a per-stone
  `GemIdentity` (instance level), origin and treatment as independent declared
  axes, gem-driven visual profiles, and a `geometryHash` distinct from
  `definitionHash` so a semantic-only edit reuses already-built geometry. No
  gemological, durability or certification claim is made anywhere. Start at
  [`23-gem-identity/README.md`](23-gem-identity/README.md).

- **Sprint 29 — Specialty Rings v1**
  ([`31-specialty-rings/`](31-specialty-rings/)), ring architectures that are
  not variants of a solitaire: a stone-set ETERNITY band, a CLUSTER and a
  TOI-ET-MOI. An EXTENSION of Ring Families v2 rather than a layer beside it —
  three new `jewelry.style` members and four new variants, resolved by the same
  single resolver, with no specialty generator anywhere. Each one DERIVES JDL
  and builds nothing: the eternity band becomes a `pave` block and the other two
  become a `family` block. Their statuses differ by MEASUREMENT rather than
  ambition: `eternity` is CURRENT because the Pavé Engine is the one layer that
  builds retention metal for a non-primary stone, and `cluster`/`toi_et_moi` are
  PARTIAL because the Multi-Stone Family layer holds only the centre. The
  sprint's audit also CORRECTED a Sprint 28 reservation that was factually
  wrong. Start at
  [`31-specialty-rings/README.md`](31-specialty-rings/README.md).

- **Sprint 28 — Ring Families v2**
  ([`30-ring-families/`](30-ring-families/)), the ring family as a PARAMETRIC
  RELATION rather than a preset: six families and thirteen variants in which
  changing the ring size, the centre stone or the band regenerates the design
  instead of leaving a stored model stale. The layer DERIVES JDL and orchestrates
  the systems that already exist — the Stone Arrangement Engine, the Halo System,
  the Pavé Engine, Setting System v2 and the Shank subsystem — and computes no
  placement, outline or solid of its own
  ([`ADR-014`](03-decisions/ADR-014-ring-family-orchestration.md)). `jewelry.style`
  stays the ONE authority for the family, so a variant belonging to another
  family is refused rather than resolved by precedence. Three real new
  components: shoulder arches — the first geometry the shoulder has ever had,
  after recording `modeled: False` since Sprint 16 — split and bypass shank
  architectures, and a signet body. PARTIAL in two specific places and for two
  different reasons: a halo's stones are real and no metal holds them, and a
  signet's table is real and nothing can decorate it. Start at
  [`30-ring-families/README.md`](30-ring-families/README.md).

- **Sprint 27 — Extended Setting Modes v1**
  ([`29-extended-setting-modes/`](29-extended-setting-modes/)), the setting
  technique as a first-class taxonomy: twenty modes across three simultaneous
  axes, with each mode's geometry claim MEASURED from the live builder
  registries rather than declared, so a fifth hand-maintained registry could
  not appear. Four new families build real solids — channel, bar, flush/gypsy
  and tension — plus a partial bezel, a pierced open-gallery head and shared
  micro-prong retention. `setting.mode` REFINES `setting.type` and a
  disagreement between them is refused rather than resolved by precedence
  ([`ADR-013`](03-decisions/ADR-013-extended-setting-modes.md)). PARTIAL in two
  specific places and for two different reasons: a tension setting's geometry is
  complete and its structural behaviour is not modelled at all, and a shared
  prong's metal is real while its position is stated rather than derived. Start
  at
  [`29-extended-setting-modes/README.md`](29-extended-setting-modes/README.md).

- **Sprint 26 — Pavé & Microsetting Engine v1**
  ([`28-pave/`](28-pave/)), surface population as a first-class field: a pavé
  states an area and a density, a microsetting states a structure, and both
  compile into arrangement primitives on a resolved host surface. THE FIRST
  SPRINT IN THIS PROGRAMME TO BUILD RETENTION METAL — beads, shared beads and
  micro-prongs are real solids fused into the host, and a recess is the stone's
  own volume cut out of it. Required a real axis tilt on the arrangement
  transform ([`ADR-011`](03-decisions/ADR-011-instance-axis-tilt.md)), because a
  stone on a curved shank must follow the surface normal. PARTIAL in scope: two
  host surfaces of seven, one field per design. Start at
  [`28-pave/README.md`](28-pave/README.md).

- **Sprint 25 — Halo System v1**
  ([`27-halo/`](27-halo/)), the halo as a first-class COMPOSABLE structure:
  single, double and hidden halos that add rings of stones to whatever
  placement a design already declares — a solitaire, a family or an explicit
  arrangement — rather than replacing it. `FamilyType` gained no member; Sprint
  24's reservation of the name is superseded on the record in
  [`27-halo/halo-rfc.md`](27-halo/halo-rfc.md). Every halo stone is real,
  individually identified, scaled and (for a hidden halo) vertically offset
  geometry. PARTIAL because no metal is generated to hold those stones. Start at
  [`27-halo/README.md`](27-halo/README.md).

- **Sprint 24 — Multi-Stone Families v1**
  ([`26-multi-stone-families/`](26-multi-stone-families/)), the semantic
  structure of multi-stone designs: three-stone, toi-et-moi, cluster and
  centre-with-accents as first-class families that COMPILE INTO arrangements
  rather than replacing them. Per-instance stone geometry became real — every
  member is placed, scaled and oriented as its own `stone_reference.<id>`
  component, required and inspected as a stone. PARTIAL because a SETTING is
  still built only for the primary stone. Start at
  [`26-multi-stone-families/README.md`](26-multi-stone-families/README.md).

- **Sprint 23 — Setting System v2: Advanced Heads & Prongs**
  ([`25-setting-v2/`](25-setting-v2/)), the single cylinder and single basket
  generalized into registry-dispatched families: 4 prong styles (round,
  tapered, claw, V), 4 head architectures (basket, peg, martini, tulip),
  explicit prong layouts with per-group style overrides, a deterministic
  setting-to-stone-instance mapping, and opt-in reference seat relief that CUTS
  the stone out of the metal rather than fusing it. Head construction became
  category-neutral. Round prongs and the basket are preserved
  character-for-character, so all 39 Goldens hold with zero updates. Start at
  [`25-setting-v2/README.md`](25-setting-v2/README.md).

- **Sprint 22 — Stone Arrangement Engine v1**
  ([`24-arrangement/`](24-arrangement/)), a stone's OCCURRENCE separated from
  what a stone is and what it is made of. First-class stone instances with
  stable ids, groups, linear/radial/mirror patterns, declared relationships,
  deterministic resolution to explicit placements, a fingerprint distinct from
  both `definitionHash` and `geometryHash`, and an explicit compilation
  boundary. Multi-stone GEOMETRY is honestly PARTIAL: every instance resolves
  and reports its generation status, one stone solid is built. Start at
  [`24-arrangement/README.md`](24-arrangement/README.md).

- **Sprint 19 — Setting System v1**
  ([`21-setting/`](21-setting/)), stone-setting geometry separated from
  ring-specific head construction into a second category-neutral
  subsystem. Prong placement became shape-aware (sampled from the
  stone's own girdle outline rather than a width-derived circle), and a
  real parametric **bezel** family was added, built by offsetting that
  same outline. Round 4/6-prong geometry is byte-identical. Start there
  at [`21-setting/README.md`](21-setting/README.md).
...
| [`14-conversation/`](14-conversation/) | Conversation Engine v1: governance, stateless multi-turn interaction model on top of Designer/Design Intent, session/turn/action/proposal/clarification models, staleness detection, current code mapping, and open questions |
| [`15-professional-validation/`](15-professional-validation/) | Professional Validation Framework v1: governance, reviewer/qualification/evidence/decision models, review case and golden review model concepts, review package contract, versioning and implementation-change-impact policy, and the active (zero-record) validation registry |
| [`16-geometry-inspection/`](16-geometry-inspection/) | Geometry Inspection v2: governance, real runtime geometric-fact reporting (component/assembly/pairwise facts, connectivity, intersection, stone-metal separation, prong count), status/error/performance models, versioning/determinism, Forge/Alchemist/Foundry/Vision integration contracts, current solitaire mapping, and open questions |
| [`17-geometry-quality/`](17-geometry-quality/) | Geometry Quality & Golden Models v1: governance, Golden Model contract, golden suite selection, quality signal/regression-comparison/tolerance models, the regression harness and explicit update policy, geometry-diff model, artifact/version-fingerprint models, CI gating, failure triage, the Professional Validation boundary, current code mapping, and open questions |
| [`18-ring-architecture/`](18-ring-architecture/) | Ring Architecture v2 / Multi-Category Ready: governance, shared-vs-category-specific domain audit, Ring architecture overview, RingDefinition/family models, sizing/shank/shoulder/head/stone-arrangement/setting-attachment contracts, component graph, generation contract, solitaire migration model, multi-category readiness and extension-test models, current code mapping, and open questions |
| [`19-shank/`](19-shank/) | Band & Shank System v1: governance, architecture overview, domain/coordinate/path/section-profile models, width/thickness function and taper models, shoulder transition and head connection interface, generation pipeline and continuity model, inspection and Forge boundary contracts, Golden strategy, current-band migration, capability model, current code mapping, and open questions |
| [`20-stone/`](20-stone/) | Stone System v1: governance, architecture overview, domain/shape/dimension/coordinate-and-orientation models, outline and reference-geometry contracts, per-family contracts (round/elongated/angular/asymmetric), generation pipeline, setting interface, inspection contract, capability model, current-round migration, Golden strategy, current code mapping, and open questions |
| [`21-setting/`](21-setting/) | Setting System v1: governance, architecture, domain model, the Stone→Setting and Setting→category attachment interfaces, prong setting contract and placement model, bezel setting contract, inspection contract, capability model, current-prong migration, Golden strategy, and code mapping/gaps |
| [`22-stone-v2/`](22-stone-v2/) | Stone System v2: governance, stone-source architecture, extended shape taxonomy and family architecture, the 14 new native cuts, the outline x profile model, the custom-outline contract and its validation, measured and imported stone contracts, import normalization, cabochon and pearl references, source provenance, Stone x Setting compatibility v2, inspection v2, Golden strategy, capability model, Stone v1 migration, code mapping/gaps and open questions |
| [`23-gem-identity/`](23-gem-identity/) | Gem Identity & Material System v1: governance, the type/instance split, gem registry and taxonomy, origin and treatment declaration models, visual profile system, geometry/identity hash separation, Designer and Conversation gem language, and the explicit non-goals (no gemological data, no certification, no measured optics) |
| [`24-arrangement/`](24-arrangement/) | Stone Arrangement Engine v1: governance, the occurrence/definition separation, stone instances, groups, closed-form patterns, declared relationships, deterministic resolution and fingerprinting, structural validation, the component-naming contract, and the multi-stone execution boundary |
| [`25-setting-v2/`](25-setting-v2/) | Setting System v2: governance, the prong-style and head-architecture registries, the category-neutral head contract, explicit prong layouts and grouping, the setting-to-stone-instance mapping, reference seat relief as a cut, and the head execution boundary |
| [`26-multi-stone-families/`](26-multi-stone-families/) | Multi-Stone Families v1: governance, the family domain contract, the four families and their semantics, compilation into arrangement primitives, per-instance stone geometry, instance-aware inspection, mixed stones and gems, and the setting-coverage execution boundary |
| [`27-halo/`](27-halo/) | Halo System v1: governance, the RFC superseding Sprint 24's reserved family name, the halo domain contract, the three variants, composition onto a family/arrangement/solitaire, the shared ring arithmetic, per-stone overrides, and the halo-metal execution boundary |
| [`28-pave/`](28-pave/) | Pavé & Microsetting Engine v1: governance, the two field models, host surface targeting and its boundary, the lattice and pattern layer, retention topology (beads, shared beads, micro-prongs), the recess contract, and the first real setting geometry in the programme |
| [`31-specialty-rings/`](31-specialty-rings/) | Specialty Rings v1: governance, the eternity/cluster/toi-et-moi families, why each status is CURRENT or PARTIAL, the coverage review of every deferred specialty form, and the execution boundary |
| [`30-ring-families/`](30-ring-families/) | Ring Families v2: governance, the six families and thirteen variants, the parametric dependency model, the derived capability registry, the three new geometry components (shoulder arches, split/bypass shank architectures, the signet body), the reserved-family coverage review, and the execution boundary |
| [`29-extended-setting-modes/`](29-extended-setting-modes/) | Extended Setting Modes v1: governance, the mode taxonomy and its three axes, the derived capability registry, the four new families (channel, bar, flush/gypsy, tension), the partial bezel and open-gallery head, shared micro-prong retention, component provenance, the taxonomy coverage audit, and the execution boundary |
...
**Technical Bible Sprint 15 — Geometry Quality & Golden Models v1**
answers a question Sprint 14 could measure but not yet track over time:
has a code change silently altered geometry? `backend/jewelmind/geometry_quality/`
(10 files, ~1,100 lines) builds a `GeometrySnapshot`/`GeometryDiff`/
`QualityResult` layer directly on top of Sprint 14's real
`GeometryInspectionReport` output — never a second geometry pipeline,
never a mock. 9 real `GoldenModel` fixtures
(`goldens/solitaire-v1/`, each a real generated `design.json` +
`snapshot.json`, no committed STEP/STL binaries) cover the default
solitaire, 4- and 6-prong variants, flat and comfort-fit band profiles,
a ring-size variation, band/stone/prong-basket dimension variations,
and one deliberately warning-only-but-valid case (a 9mm stone with 4
prongs, triggering Forge's `JM-PRONG-003`). Its most important
architectural finding is empirical, not theoretical: Sprint 14's own CI
run had already measured a real ~1.3e-5 relative cross-platform
floating-point divergence on a near-tangent sliver intersection volume;
Sprint 15's `RELATIVE_COMPARISON_TOLERANCE` (1e-3) is set two orders of
magnitude above that measured value, not guessed. Baseline updates are
structurally impossible to automate — `verify_golden()`,
`verify_all_goldens()`, and `generate_candidate_baseline()` never call
the one function that writes an accepted baseline
(`accept_candidate_baseline()`, reachable only via
`geometry-quality accept --reason "..."`), enforced by real tests that
...
**Technical Bible Sprint 16 — Ring Architecture v2 / Multi-Category
Ready** formalizes a distinction the code had never stated explicitly:
JewelMind is the platform, Ring is one jewelry category, Solitaire is
one ring family — and `JewelMind == RingEngine` must never become true.
`backend/jewelmind/jewelry_category/` (6 files) owns category identity,
a real `CategoryCapability` registry (`ring`: current, generatable;
`earring`/`pendant`/`bracelet`/`necklace`/`charm`: recognized but
planned, never advertised as working), and a generic dispatch function
that has never heard of a ring field. `backend/jewelmind/ring/` (4
files) composes a `RingDefinition` v2 from real, reusable sub-models
(sizing, shank, shoulders, head, stone arrangement, setting attachment)
built by a pure adapter, `ring_definition_from_jdl()`, from the
UNMODIFIED real JDL schema — no breaking change, no second canonical
JDL schema. `ModelService.generate()` and
`geometry_quality/snapshot.py` now both dispatch through
`jewelry_category.dispatch.generate_jewelry()` rather than calling
`build_solitaire_ring()` directly, but that function itself — Sprint
5's real geometry builder — was not touched, and the full Golden Suite
(Sprint 15) passed with **zero baseline updates**, proving the refactor
changed architecture, not geometry. Its most important architectural
finding, discovered empirically during implementation rather than
planned: a genuine circular import between `jewelmind.ring` and
`jewelmind.jewelry_category` (each importing the other's submodules
eagerly, before either package finished initializing) — reproduced from
both import orders, then fixed by deferring the cross-package registry
build into a cached function called only on first real dispatch. A
test-only `DummyPendantDefinition`
(`backend/tests/test_jewelry_category_extension.py`) proves the
dispatch boundary is genuinely generic — it is never added to
`CATEGORY_CAPABILITIES`, any production generator registry, Designer's
capabilities, or the JDL schema. Adds the machine-readable
specification under
[`../../specs/jewelry-architecture/v1/`](../../specs/jewelry-architecture/v1/README.md)
(3 JSON Schemas, a 6-entry category registry) and
[`../../specs/ring/v2/`](../../specs/ring/v2/README.md) (8 JSON
Schemas, 3 examples, 4 test-vector files) plus 3 new test files:
`test_jewelry_category_extension.py`, `test_ring_architecture.py`, and
`test_ring_architecture_schemas.py`.

**Technical Bible Sprint 17 — Band & Shank System v1** replaces the
old single-file, uniform-only `geometry/components/band.py` with a
reusable, parametric `geometry/shank/` subsystem (profile, taper,
builder, capability modules) — the first Sprint since Atlas (Sprint 5)
to add real, new CAD geometry rather than architecture or reporting.
`domain/schema.py::BandSpec` gains `widthTaper`/`thicknessTaper`
(`BandTaperSpec`, `mode: "NONE" | "TOWARD_BOTTOM"`) as an additive,
backward-compatible MINOR JDL change — `schemaVersion` stays `"0.1.0"`.
Construction dispatches on whether taper is actually requested: no
taper produces the byte-identical pre-Sprint-17 `revolve()` construction
(zero Golden regression for all 9 existing cases); any taper produces a
real 48-section `cq.Solid.makeLoft()`, with `SECTION_COUNT=48` chosen
from measured volume convergence across 16–72 sections. Taper is always
anchored at the head (`u=0`, matching the existing `band_top_z()`
reference point) so the new, explicit `geometry/connection.py::ShankConnectionInterface`
(`topZMm`/`embedMm`/`headCenterRadiusMm`) — the named Shank → RingHead
handoff `prongs.py`/`basket.py` now consume — never moves for any taper
configuration, requiring zero changes to prong/basket placement logic.
The tapered path does not yet apply the outer-rim fillet (a real,
documented v1 limitation, not a silent gap). Two real bugs were found
and fixed during implementation: a circular import from initially
placing the connection-interface module inside `jewelmind/ring/`
instead of the Atlas layer (`jewelmind/geometry/`), and Designer's
`flatten_definition()` flattening JDL only one level deep, breaking on
the new nested `BandTaperSpec` object across 260 previously-unrelated
tests until fixed with a recursive rewrite. Adds the machine-readable
specification under
[`../../specs/shank/v1/`](../../specs/shank/v1/README.md) (6 JSON
Schemas, a real generated capability registry, 5 examples, 6 test-vector
files) plus 2 new test files: `test_shank.py` (77 tests) and
`test_shank_schemas.py` (8 tests).

**Technical Bible Sprint 18 — Stone System v1** generalizes the
round-only `StoneReference` into shared, **category-neutral** jewelry
infrastructure — the first subsystem to prove that Sprint 16's
category boundary is real rather than aspirational. All 7 target shapes
(`round`, `oval`, `pear`, `emerald`, `cushion`, `princess`, `marquise`)
genuinely generate deterministic CAD reference geometry via one shared
3-level crown/girdle/pavilion ruled loft over 7 per-shape 2D outline
primitives (`backend/jewelmind/geometry/stone/`), with `round`
preserved byte-identically in its own builder and **zero Golden
baseline updates** across all 12 pre-existing cases. `StoneSpec` gains
6 enum members plus `length`/`width`/`orientation` as an additive MINOR
JDL change (`schemaVersion` stays `"0.1.0"`), and
`backend/jewelmind/domain/stone_dimensions.py` resolves the per-shape
public fields into one canonical LENGTH/WIDTH/DEPTH contract — placed
in `domain/` specifically so both Atlas geometry and Forge validation
can consume it without creating a new cross-layer coupling. Geometry
Inspection gains 6 real `STONE_*` fact types that deliberately pair a
CONSTRUCTION_PARAMETER with an independently MEASURED_GEOMETRY
counterpart, so an accidental scaling shows up as a divergence between
them. Its most important discipline is a refusal: **no fake equivalent
diameter was introduced** — `JM-STONE-001` and `JM-PRONG-003` are
honestly scoped ROUND_ONLY rather than evaluated against a substituted
dimension, while `JM-STONE-002` was genuinely generalized to the
stone's real minimum horizontal extent. Stone-generation capability is
kept rigorously separate from Setting compatibility: all 7 shapes
generate, but only `round` is `SUPPORTED` and the other 6 are honestly
`EXPERIMENTAL`, because the current prong layout is a generic circular
placement that leaves marquise/pear tips and angular-stone corners
unsupported. `backend/tests/test_stone_system_no_ring_dependency.py`
proves category-neutrality by AST-parsing every Stone System file
rather than importing it (an import-based check can pass by accident on
a cached module). A real circular import was found and fixed during
implementation — `geometry/constants.py` needs `resolved_width_mm`
while `geometry/stone/builder.py` needs `band_top_z`, so
`geometry/stone/__init__.py` is deliberately non-eager. Adds the
machine-readable specification under
[`../../specs/stone/v1/`](../../specs/stone/v1/README.md) (5 JSON
Schemas, a real generated shape registry, 7 examples, 5 test-vector
files) plus 3 new test files: `test_stone.py` (92 tests),
`test_stone_schemas.py` (18 tests), and
`test_stone_system_no_ring_dependency.py` (8 tests).

**Technical Bible Sprint 19 — Setting System v1** extracts stone-setting
geometry from the solitaire vertical slice into a second
**category-neutral** subsystem, `backend/jewelmind/setting/`. Before this
Sprint "the setting" was whatever `geometry/components/prongs.py`
happened to do: it read ring fields directly, derived placement from
`stone.diameter`, and defined by accident what every future setting
would have to look like. Now the dependency arrow is one-way and
enforced — `Ring → geometry/setting_adapter.py → jewelmind.setting →
Stone contracts` — with `jewelmind.setting` forbidden from importing
`jewelmind.ring`, `jewelmind.jewelry_category`, the Shank subsystem, or
even `JewelryDefinition` (which would smuggle the whole ring domain
across in one import), proven by AST inspection in
`test_setting_system_no_ring_dependency.py`. A `RingHead` consumes a
`SettingAttachmentInterface`; a future `PendantBody` consumes the same
contract. **Prong placement became shape-aware**: `RADIAL` for a
radially symmetric stone (byte-identical to before) and
`OUTLINE_CARDINAL` — the support function of the stone's own girdle
outline — for every other shape, which moved an oval's off-axis prongs
from floating 0.784 mm clear of the stone to 0.049 mm from it while
leaving the on-axis prong untouched. A real parametric **bezel** family
was added, built by offsetting that same outline, so the pipeline is
outline-agnostic rather than per-shape; all 7 stone shapes produce a
valid single-solid wall. Its defining finding: a true geometric offset
of an **ellipse** produces `OFFSET`-type edges whose extruded surface
does **not** survive a STEP round-trip — it re-imports as a `Shell` with
zero solids — so the repair is triggered by the real **curve type**
rather than a shape name, which keeps it general and leaves the angular
shapes' crisp corners untouched. Round 4/6-prong geometry is exactly
preserved (`341.44334316909976 mm³`) with **zero** baseline updates
across all 12 round-stone Golden cases; the six non-round cases changed
**intentionally** and were accepted through the full
`generate-candidate → diff → accept --reason` path, the programme's
first reviewed baseline change. Five real architectural leaks were found
by exercising a bezel rather than by reading code — including an
unconditional `components["prongs"]` index in `api/routes.py` that would
have made **every bezel generation crash the API**, and a hardcoded
inspection-pair tuple that silently excluded the `bezel` component from
all pairwise inspection. Adds `specs/setting/v1/` (7 JSON Schemas, a
generated registry with the full Stone × Setting matrix, 5 examples, 5
test-vector files), the cross-product **Capability Coverage Guard** at
`specs/capabilities/` (101 capabilities across 26 domains, validated
against real code), and 4 new test files: `test_setting.py` (102 tests),
`test_setting_schemas.py` (22), `test_setting_system_no_ring_dependency.py`
(13), and `test_capability_coverage.py` (16).
