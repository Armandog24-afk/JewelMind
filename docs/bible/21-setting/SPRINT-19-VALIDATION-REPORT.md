...
## Cross-platform float drift in the backward-compatibility assertion

Caught by CI, not locally. The two `ROUND_*_PRONG_REGRESSION` guards asserted
**exact** float equality on `combined_metal_volume_mm3`, which is the volume of
the OCCT **boolean fuse** of band + basket + prongs. That value is
`341.44334316909976` on Windows and `341.44334316907685` on CI's Linux build —
