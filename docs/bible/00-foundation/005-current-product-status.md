...
| Flat band geometry | current | `backend/jewelmind/geometry/components/band.py` | `test_geometry.py::test_flat_band_is_valid_solid_with_positive_volume` | Optional outer-rim fillet has a documented fallback path (`docs/known-limitations.md`). | None planned. |
| Comfort-fit band geometry | current | `backend/jewelmind/geometry/components/band.py` | `test_geometry.py::test_comfort_fit_band_is_valid_solid_with_positive_volume`, `test_flat_and_comfort_fit_bands_differ_in_volume` | Fixed flare constant, not user-adjustable. | Expose flare amount as a parameter if requested. |
