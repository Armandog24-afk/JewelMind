...
| `setting.headArchitecture` | enum | `BASKET` (the pre-Sprint-23 wall, and the default), `PEG_HEAD`, `MARTINI`, `TULIP`. The generated component is named `basket_support` for every architecture — the name is a structural role, and the architecture is reported separately. `TRELLIS` is deliberately not a member. |
...
| `family.familyType` | enum | `THREE_STONE`, `TOI_ET_MOI`, `CLUSTER`, `CENTER_WITH_ACCENTS`. Each has a real compiler; `halo`/`pave`/`eternity`/`bypass` are deliberately not members (a halo is already a `CENTER_WITH_ACCENTS`). |
